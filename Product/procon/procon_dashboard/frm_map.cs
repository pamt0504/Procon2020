...
namespace procon_dashboard
{
    public partial class frm_map : Form
    {
        ...
        private void DrawMap(int hang, int cot, List<List<int>> tiled, List<List<int>> points, int myTeam, List<Obstacles> obstacles, List<Treasure> treasures, List<Team> _teamID)
        {
            exportMap(cot, hang, points, treasures, obstacles, _teamID);
        ...
        public async Task MatchesID(string url, string token, int id)
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                if (result.IsSuccessStatusCode)
                {
                    ...
                    DrawMap(data.height, data.width, data.tiled, data.points, myTeamId, data.obstacles, data.treasure, data.teams);
                    //ve agent hien tai
                    foreach (var item in data.teams)
                    {
                        //if (item.teamID == myTeamId)
        ...
        private void exportMap(int height, int width, List<List <int>> points, List<Treasure> treasure, List<Obstacles> obs, List<Team> team)
        {
            StreamWriter sw = new StreamWriter("D:\\Procon\\Procon_2020\\Product\\procon\\logTest.txt");
            sw.WriteLine(height + " " + width);
            int[,] ar1 = new int[data.height+1, data.width+1];
            int hoanh = 0, tung = 0;
            ...
            foreach (var s in treasure)
            {
                if (s.status == 0)
                {
                    ar1[s.y-1, s.x-1] = s.point;
                }
            }
            foreach (var s in obs)
            {
                ar1[s.y-1, s.x-1] = Int32.MinValue;
            }
            for (int i = 0; i < tung; i++)
            {
                for (int j = 0; j < hoanh; j++)
                {
                    sw.Write(ar1[i, j] +  "\t");
                }
                sw.WriteLine();
            ...
            {
                foreach (var id in item.agents)
                {
                    sw.WriteLine(id.agentID + " " + (id.x-1) +" "+ (id.y-1));
                }
            }
            sw.Flush();
